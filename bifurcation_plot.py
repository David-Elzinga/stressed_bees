...
plt.xlim(0, 0.15); plt.ylim(0, 0.4)
plt.xlabel(r'$\gamma$',fontsize=20); plt.ylabel(r'$\beta$',rotation=0, labelpad = 30, fontsize=20)
plt.legend(fontsize=10)

plt.text(0.12, 0.26, "Persistence", fontsize=10)
plt.text(0.07, 0.25, "Extinction \nby Stressor", fontsize=10, horizontalalignment='center')
plt.text(0.02, 0.25, "Forced \nExtinction", fontsize=10, horizontalalignment='center')

plt.plot([0.0625, 0.0625], [0.05, 0.15], 'r.',markersize=20) # adds the red dots. 
...
for n, parm['beta'] in enumerate([0.05, 0.15]):
    ...
    ax[n].set_xlabel(r'$t$ (days)', fontsize=14)
    ax[n].set_ylabel('Bees', fontsize=14)
    ax[n].set_title(r'$\beta = $' + str(parm['beta']), fontsize=14)
